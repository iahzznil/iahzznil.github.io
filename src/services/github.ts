...
class Github {
  ...
  constructor(private token1: string, private token2: string, private owner: string, private repo: string) {}
  ...
  private async request(method: string, url: string, data?: Record<string, unknown>) {
    ...
    const response = await fetch([this.apiBase, url, query].join(''), {
      ...
      headers: {
        ...
        Authorization: `token ${this.token1}${this.token2}`,
      },
...
export default new Github(
  ...
  import.meta.env.VITE_GITHUB_ACCESS_TOKEN_PART2,
